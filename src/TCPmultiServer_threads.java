...
public class TCPmultiServer_threads implements Runnable { // TCPmultiServer_threads extends Thread -> can be done either
															// way
	...
	public static void main(String args[]) throws IOException {
		...
		ServerSocket listener = ((SSLServerSocketFactory)SSLServerSocketFactory.getDefault()).createServerSocket(3333);
		...
		} // so that we could accept other clients on ServerSocket listener at the same
			// time
	}
	...
	public void run() { // executed by every client thread
		try {
			...
			// autoflush - A value indicating whether the stream should be automatically
			// flushed when it is written to.
			// equivalent to manual flush: out.flush(); -> This forces data to be sent to
			// the server without closing the Socket.
			String query;
			while (true) {
